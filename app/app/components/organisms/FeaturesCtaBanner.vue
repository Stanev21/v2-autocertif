<template>
  <section class="relative py-32 lg:py-40 overflow-hidden">
    ...
    <!-- Animated orbs -->
    <div class="absolute top-1/2 left-1/4 -translate-y-1/2 w-[250px] sm:w-[500px] h-[250px] sm:h-[500px] rounded-full opacity-20 blur-[120px]"
      style="background: radial-gradient(circle, #3B82F6, transparent 70%);"></div>
    <div class="absolute bottom-0 right-1/4 w-[200px] sm:w-[400px] h-[200px] sm:h-[400px] rounded-full opacity-10 blur-[100px]"
    ...
    <div class="relative max-w-[900px] mx-auto px-4 sm:px-8 lg:px-12 text-center">
      <!-- Badge -->
      <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-white/10 bg-white/[0.03] backdrop-blur-sm mb-8">
        ...
        <span class="text-white/50 text-[13px] font-inter font-medium">{{ $t('features.badge') }}</span>
      </div>

      <h2 class="text-[2rem] sm:text-[2.5rem] md:text-[3.25rem] lg:text-[4rem] font-normal text-white leading-[1.08] mb-6 font-heading tracking-[-0.02em]">
        {{ $t('features.ctaTitle') }}
      </h2>
      <p class="text-white/40 text-[1.1rem] leading-[1.6] font-inter mb-12 max-w-xl mx-auto">
      ...
      <!-- Bottom stats -->
      <div class="grid grid-cols-2 sm:flex sm:items-center sm:justify-center gap-4 sm:gap-6 mt-10 sm:mt-14">
        <div class="flex items-center gap-2">
          <svg class="w-4 h-4 text-green-400 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 1 0 0-16 8 8 0 0 0 0 16Zm3.857-9.809a.75.75 0 0 0-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 1 0-1.06 1.061l2.5 2.5a.75.75 0 0 0 1.137-.089l4-5.5Z" clip-rule="evenodd" /></svg>
          <span class="text-white/40 text-[12px] sm:text-[13px] font-inter">{{ $t('features.ctaSources') }}</span>
        </div>
        <div class="w-px h-3 bg-white/10 hidden sm:block"></div>
        <div class="flex items-center gap-2">
          <svg class="w-4 h-4 text-green-400 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 1 0 0-16 8 8 0 0 0 0 16Zm3.857-9.809a.75.75 0 0 0-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 1 0-1.06 1.061l2.5 2.5a.75.75 0 0 0 1.137-.089l4-5.5Z" clip-rule="evenodd" /></svg>
          <span class="text-white/40 text-[12px] sm:text-[13px] font-inter">{{ $t('features.ctaCheckpoints') }}</span>
        </div>
        <div class="w-px h-3 bg-white/10 hidden sm:block"></div>
        <div class="flex items-center gap-2">
          <svg class="w-4 h-4 text-green-400 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 1 0 0-16 8 8 0 0 0 0 16Zm3.857-9.809a.75.75 0 0 0-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 1 0-1.06 1.061l2.5 2.5a.75.75 0 0 0 1.137-.089l4-5.5Z" clip-rule="evenodd" /></svg>
          <span class="text-white/40 text-[12px] sm:text-[13px] font-inter">{{ $t('features.ctaCountries') }}</span>
        </div>
        <div class="w-px h-3 bg-white/10 hidden sm:block"></div>
        <div class="flex items-center gap-2">
          <svg class="w-4 h-4 text-green-400 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 1 0 0-16 8 8 0 0 0 0 16Zm3.857-9.809a.75.75 0 0 0-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 1 0-1.06 1.061l2.5 2.5a.75.75 0 0 0 1.137-.089l4-5.5Z" clip-rule="evenodd" /></svg>
          <span class="text-white/40 text-[12px] sm:text-[13px] font-inter">{{ $t('features.ctaReports') }}</span>
        </div>
      </div>
    </div>
  </section>
