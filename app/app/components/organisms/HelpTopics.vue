<template>
  <section class="py-16 sm:py-20 lg:py-24 bg-white">
    <div class="max-w-[1280px] mx-auto px-4 sm:px-8 lg:px-12">
      <h2 class="text-[1.75rem] lg:text-[2rem] font-heading font-normal text-gray-900 mb-12 lg:mb-14">
        {{ $t('help.topicsTitle') }}
      </h2>

      <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
        <NuxtLink
          v-for="(topic, i) in topics"
          :key="i"
          :to="localePath('/help-center/' + topicSlugs[i])"
          class="group relative rounded-2xl border border-gray-100 p-7 hover:border-blue-200 hover:bg-blue-50/30 transition-all duration-300 cursor-pointer"
          @mouseenter="hoveredTopic = i"
          @mouseleave="hoveredTopic = -1"
        >
          <!-- Arrow icon -->
          <div class="absolute top-5 right-5 text-gray-200 group-hover:text-blue-500 transition-colors duration-300">
            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" d="m4.5 19.5 15-15m0 0H8.25m11.25 0v11.25" />
            </svg>
          </div>

          <!-- Icon -->
          <div class="w-12 h-12 rounded-xl flex items-center justify-center mb-5 transition-colors duration-300"
            :class="hoveredTopic === i ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-400'"
          >
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" :d="topicIcons[i]" />
            </svg>
          </div>

          <h3 class="text-[1rem] font-inter font-semibold text-gray-900 mb-2 group-hover:text-blue-700 transition-colors">
            {{ topic.title }}
          </h3>
          <p class="text-gray-400 text-[13px] leading-[1.65] font-inter">{{ topic.desc }}</p>

        </NuxtLink>
      </div>
    </div>
  </section>
</template>

<script setup lang="ts">
const { tm, rt } = useI18n()
const localePath = useLocalePath()
const hoveredTopic = ref(-1)

const topics = computed(() => {
  const raw = tm('help.topics')
  return Array.isArray(raw) ? raw.map((t: any) => ({ title: rt(t.title), desc: rt(t.desc) })) : []
})

const topicSlugs = [
  'getting-started',
  'reports-data',
  'vehicle-information',
  'account-billing',
  'business-solutions',
  'privacy-security',
]

const topicIcons = [
  'M4.26 10.147a60.438 60.438 0 0 0-.491 6.347A48.62 48.62 0 0 1 12 20.904a48.62 48.62 0 0 1 8.232-4.41 60.46 60.46 0 0 0-.491-6.347m-15.482 0a50.636 50.636 0 0 0-2.658-.813A59.906 59.906 0 0 1 12 3.493a59.903 59.903 0 0 1 10.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.717 50.717 0 0 1 12 13.489a50.702 50.702 0 0 1 7.74-3.342M6.75 15a.75.75 0 1 0 0-1.5.75.75 0 0 0 0 1.5Zm0 0v-3.675A55.378 55.378 0 0 1 12 8.443m-7.007 11.55A5.981 5.981 0 0 0 6.75 15.75v-1.5',
  'M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z',
  'M8.25 18.75a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 0 1-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124a17.902 17.902 0 0 0-3.213-9.193 2.056 2.056 0 0 0-1.58-.86H14.25M16.5 18.75h-2.25m0-11.177v-.958c0-.568-.422-1.048-.987-1.106a48.554 48.554 0 0 0-10.026 0 1.106 1.106 0 0 0-.987 1.106v7.635m12-6.677v6.677m0 4.5v-4.5m0 0h-12',
  'M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 0 0 2.25-2.25V6.75A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25v10.5A2.25 2.25 0 0 0 4.5 19.5Z',
  'M3.75 21h16.5M4.5 3h15M5.25 3v18m13.5-18v18M9 6.75h1.5m-1.5 3h1.5m-1.5 3h1.5m3-6H15m-1.5 3H15m-1.5 3H15M9 21v-3.375c0-.621.504-1.125 1.125-1.125h3.75c.621 0 1.125.504 1.125 1.125V21',
  'M9 12.75 11.25 15 15 9.75m-3-7.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285Z',
]
</script>
